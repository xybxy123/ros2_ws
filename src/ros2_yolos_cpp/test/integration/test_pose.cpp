...
// Integration test for PoseAdapter and PoseNode
...
protected:
  ...
  static void SetUpTestSuite() {
    model_path_ = getEnvOrDefault("YOLOS_POSE_MODEL", "");
...
std::string PoseTest::model_path_;
std::string PoseTest::test_image_path_;

TEST_F(PoseTest, AdapterFactoryCreatesInstance) {
  ASSERT_NE(createPoseAdapter(), nullptr);
}

TEST_F(PoseTest, AdapterInitialization) {
  if (!hasTestResources()) GTEST_SKIP();
  auto adapter = createPoseAdapter();
  YolosConfig config;
  config.model_path = model_path_;
  ASSERT_TRUE(adapter->initialize(config));
}

TEST_F(PoseTest, NodeLifecycleConfiguration) {
  if (!hasTestResources()) GTEST_SKIP();
  ...
  auto node = std::make_shared<YolosPoseNode>(options);
